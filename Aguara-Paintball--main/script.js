...
async function loadEditableMedia() {
  try {
    ...
    if (!data.ok || !Array.isArray(data.media)) return;

    const bySlot = Object.fromEntries(
      data.media.map((item) => [item.slot, item])
    );

    const hero = bySlot.hero_video;
    const heroVideo = document.getElementById("heroVideo");
    const heroSource = document.getElementById("heroVideoSource");

    if (hero?.url && heroVideo && heroSource) {
      heroSource.src = hero.url;
      heroSource.type = hero.mime_type || "video/mp4";
      heroVideo.load();
      heroVideo.play().catch(() => {});
    }

    document.querySelectorAll("[data-media-slot]").forEach((item) => {
      const media = bySlot[item.dataset.mediaSlot];
      if (!media?.url) return;

      const image = item.querySelector("img");
      if (image) {
        image.src = media.url;
        image.alt = media.alt_text || "Aguará Paintball";
      }
    ...
    if (featureImage && bySlot.gallery_1?.url) {
      featureImage.style.backgroundImage =
        `url("${bySlot.gallery_1.url.replaceAll('"', '\\"')}")`;
    }
  } catch (error) {
    console.warn("Medios editables no disponibles; se usan los originales.", error);
  }
}
