...
(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .aguara-day{position:relative;aspect-ratio:1/1;width:100%;min-height:0;border:1px solid rgba(255,255,255,.14);border-radius:8px;background:#6f9f55;color:#111;cursor:pointer;padding:3px;display:flex;align-items:center;justify-content:center;transition:transform .15s ease,border-color .15s ease,background .15s ease;overflow:hidden}
      .aguara-day:hover:not(:disabled){transform:translateY(-1px);border-color:rgba(255,255,255,.45)}
      .aguara-day:disabled{cursor:default}
      .aguara-day.is-outside{opacity:.22}
      .aguara-day.is-today{box-shadow:inset 0 0 0 1px rgba(255,255,255,.65)}
      .aguara-day.is-selected{background:#f2f2f2;color:#111;border-color:#fff;box-shadow:0 0 0 2px rgba(255,255,255,.28)}
      .aguara-day.is-reserved{background:#a33b3b;border-color:#c95a5a;color:#fff}
      .aguara-day-number{position:relative;z-index:2;font-size:.82rem;font-weight:900;line-height:1}
      .aguara-day-x{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:3;color:#7b1010;font-size:1.35rem;line-height:1;font-weight:900;text-shadow:0 0 2px rgba(255,255,255,.45)}
      .aguara-day-label{position:absolute;bottom:2px;left:50%;transform:translateX(-50%);font-size:.34rem;font-weight:900;color:rgba(255,255,255,.92);white-space:nowrap;letter-spacing:.02em}
      .aguara-day:not(.is-reserved):not(.is-selected) .aguara-day-label{color:rgba(0,0,0,.72)}
      .aguara-calendar-status{margin-top:10px;padding:8px 10px;border-radius:9px;background:rgba(0,0,0,.30);font-size:.74rem;color:rgba(255,255,255,.82)}
      .aguara-time-panel{margin-top:11px;padding-top:11px;border-top:1px solid rgba(255,255,255,.10)}
      .aguara-time-title{margin:0 0 8px;font-size:.82rem;font-weight:800}
      .aguara-time-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px}
      .aguara-time-card{position:relative;border:1px solid rgba(255,255,255,.14);border-radius:8px;background:#6f9f55;color:#111;padding:7px 8px;text-align:left;cursor:pointer;font-weight:800;font-size:.78rem}
      .aguara-time-card small{display:block;margin-top:1px;font-size:.55rem;font-weight:700;color:rgba(0,0,0,.70)}
      .aguara-time-card.is-selected{border-color:#fff;background:#f2f2f2;color:#111;box-shadow:0 0 0 2px rgba(255,255,255,.18)}
      .aguara-time-card.is-selected small{color:rgba(0,0,0,.70)}
      .aguara-time-card.is-reserved{background:#a33b3b;border-color:#c95a5a;color:#fff;cursor:not-allowed;padding-top:15px}
      .aguara-time-card.is-reserved::before{content:"✕";position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#7b1010;font-size:1.15rem;font-weight:900;text-shadow:0 0 2px rgba(255,255,255,.45)}
      .aguara-time-card.is-reserved small{color:rgba(255,255,255,.92)}
      @media(max-width:640px){.aguara-calendar-shell{padding:10px;border-radius:12px}.aguara-calendar-weekdays,.aguara-calendar-grid{gap:4px}.aguara-day{border-radius:7px}.aguara-day-number{font-size:.76rem}.aguara-day-x{font-size:1.15rem}.aguara-day-label{font-size:.30rem}.aguara-time-grid{gap:5px}.aguara-time-card{padding:6px 7px;font-size:.72rem}}
    `;
