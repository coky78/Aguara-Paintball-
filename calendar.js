/* =====================================================
   ...
   CORRECCIÓN:
   - En el día actual oculta horarios que ya pasaron.
   - Mantiene intactas las reservas y horarios futuros.
   - Al seleccionar una fecha, desplaza suavemente hasta los horarios.
===================================================== */
...
(() => {
  ...
  function timeHasPassed(date, time) {
    const today = dateKey(new Date());
    if (date !== today) return false;
    const current = currentTime5();
    return time <= current;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .aguara-time-card.is-past{display:none}
  ...
  function makeShell() {
    ...
    shell.innerHTML = `
      <div class="aguara-calendar-head"><h3 class="aguara-calendar-title" id="aguaraCalendarTitle"></h3><div class="aguara-calendar-nav"><button type="button" id="aguaraPrevMonth" aria-label="Mes anterior">‹</button><button type="button" id="aguaraNextMonth" aria-label="Mes siguiente">›</button></div></div>
      <div class="aguara-calendar-weekdays"><div class="aguara-calendar-weekday">Dom</div><div class="aguara-calendar-weekday">Lun</div><div class="aguara-calendar-weekday">Mar</div><div class="aguara-calendar-weekday">Mié</div><div class="aguara-calendar-weekday">Jue</div><div class="aguara-calendar-weekday">Vie</div><div class="aguara-calendar-weekday">Sáb</div></div>
      ...
      <div class="aguara-time-panel"><h4 class="aguara-time-title">Horarios disponibles</h4><div class="aguara-time-grid" id="aguaraTimeGrid"></div></div>
    `;
    ...
    dateInput.addEventListener("change",()=>{state.selectedDate=dateInput.value;if(state.selectedDate){const d=parseDate(state.selectedDate);state.month=new Date(d.getFullYear(),d.getMonth(),1);}renderCalendar();renderTimes();});
  ...
  function renderCalendar() {
    ...
    for (let i=0;i<firstDay;i++) {
      const cell=document.createElement("button");cell.type="button";cell.disabled=true;cell.className="aguara-day is-outside";cell.textContent=String(new Date(y,m,0).getDate()-firstDay+i+1);grid.appendChild(cell);
    }
    const today=dateKey(new Date());
    for (let d=1;d<=days;d++) {
      const key=`${y}-${pad(m+1)}-${pad(d)}`;
      const count=key>=today?(state.byDate.get(key)?.size||0):0;
      const btn=document.createElement("button");btn.type="button";btn.className=`aguara-day ${key===today?"is-today":""} ${key===state.selectedDate?"is-selected":""} ${count?"is-reserved":""}`;btn.title=count?`${count} horario(s) reservado(s)`:"Día disponible";btn.innerHTML=`<span class="aguara-day-number">${d}</span>`;btn.addEventListener("click",()=>selectDate(key));grid.appendChild(btn);
    }
  ...
  function selectDate(key) {
    ...
    // Después de elegir la fecha, llevar al cliente directamente
    // al bloque de horarios disponibles.
    const timePanel=document.querySelector(".aguara-time-panel");
    if(timePanel){
  ...
  function renderTimes() {
    ...
    const date=document.getElementById("date")?.value||state.selectedDate;
    const options=Array.from(select.options).filter(o=>o.value);
    grid.innerHTML="";
    if(!date||!options.length){grid.innerHTML=`<div style="grid-column:1/-1;color:rgba(255,255,255,.52);font-size:.82rem;">Primero elegí una fecha.</div>`;return;}
    let visibleTimes=0;
    for(const option of options){
      const time=time5(option.value);
      const isReserved=reserved(date,time);
      const isPast=timeHasPassed(date,time);
      if(isPast)continue;
      visibleTimes++;
      const card=document.createElement("button");card.type="button";card.className=`aguara-time-card ${option.value===select.value&&!isReserved?"is-selected":""} ${isReserved?"is-reserved":""}`;card.disabled=isReserved;card.innerHTML=`<span>${time}</span><small>${isReserved?"RESERVADO":"DISPONIBLE"}</small>`;
      if(!isReserved){card.addEventListener("click",()=>{select.value=option.value;select.dispatchEvent(new Event("change",{bubbles:true}));});}
      grid.appendChild(card);
    }
    if(visibleTimes===0){grid.innerHTML=`<div style="grid-column:1/-1;padding:14px;border-radius:11px;background:#25170b;border:1px solid #704318;color:#ffc98d;font-size:.84rem;line-height:1.5;">No quedan horarios disponibles para hoy. Elegí otra fecha.</div>`;}
  }
  ...
  function init(){
    ...
    const d=parseDate(today);state.month=new Date(d.getFullYear(),d.getMonth(),1);
    renderCalendar();renderTimes();loadReservations();
