...
(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      .aguara-calendar-shell{grid-column:1/-1;margin:0 0 7px;padding:10px;border:1px solid rgba(255,255,255,.10);border-radius:14px;background:linear-gradient(145deg,rgba(20,15,12,.97),rgba(10,10,10,.97));box-shadow:0 18px 45px rgba(0,0,0,.25)}
      .aguara-calendar-head{display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:10px}
      .aguara-calendar-title{margin:0;text-transform:capitalize;font-weight:800;font-size:.95rem;letter-spacing:.02em}
      .aguara-calendar-nav{display:flex;gap:6px}
      .aguara-calendar-nav button{width:30px;height:30px;border:1px solid rgba(255,255,255,.14);border-radius:12px;background:#231812;color:#fff;cursor:pointer;font-size:.95rem;font-weight:800}
      .aguara-calendar-nav button:hover{background:#3b2418}
      .aguara-calendar-weekdays,.aguara-calendar-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:5px}
      .aguara-calendar-weekdays{margin-bottom:7px}
      .aguara-calendar-weekday{padding:3px 0;text-align:center;font-size:.62rem;color:rgba(255,255,255,.55);font-weight:800;text-transform:uppercase}
      .aguara-day{position:relative;min-height:42px;border:1px solid rgba(255,255,255,.10);border-radius:12px;background:#000;color:#fff;cursor:pointer;padding:6px 4px;display:flex;align-items:center;justify-content:center;transition:transform .15s ease,border-color .15s ease,background .15s ease}
      ...
      .aguara-day.is-available{background:rgba(56,142,60,.78);color:#000;border-color:rgba(76,175,80,.9)}
      .aguara-day.is-reserved-day{background:rgba(198,40,40,.82);color:#fff;border-color:rgba(244,67,54,.9);overflow:hidden}
      .aguara-day.is-reserved-day .aguara-day-number{position:relative;z-index:1;color:#fff}
      .aguara-day.is-reserved-day .aguara-day-x{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:rgba(139,0,0,.80);font-size:1.65rem;font-weight:900;line-height:1;z-index:2;pointer-events:none}
      .aguara-day.is-outside{opacity:.28}
      .aguara-day.is-today{box-shadow:inset 0 0 0 1px rgba(255,255,255,.48)}
      .aguara-day.is-selected{border-color:#ffffff;box-shadow:0 0 0 2px rgba(255,255,255,.32);background:#f2f2f2;color:#000}
      .aguara-day.is-reserved{background:rgba(84,47,26,.72);border-color:rgba(126,67,34,.90)}
      .aguara-day-number{position:relative;z-index:2;font-size:.80rem;font-weight:900;margin-top:8px}
      .aguara-day-x{position:absolute;top:3px;left:50%;transform:translateX(-50%);z-index:1;color:#ff3131;font-size:.95rem;line-height:1;text-shadow:0 1px 5px rgba(0,0,0,.9)}
      ...
      .aguara-calendar-status{margin-top:8px;padding:6px 8px;border-radius:11px;background:rgba(0,0,0,.23);font-size:.76rem;color:rgba(255,255,255,.78)}
      .aguara-time-panel{margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.08)}
      .aguara-time-title{margin:0 0 7px;font-size:.80rem;font-weight:800}
      .aguara-time-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px}
      .aguara-time-card{position:relative;border:1px solid rgba(255,255,255,.12);border-radius:10px;background:rgba(56,142,60,.78);color:#000;padding:7px 8px;text-align:left;cursor:pointer;font-weight:800}
      .aguara-time-card small{display:block;margin-top:2px;font-size:.58rem;font-weight:700;color:rgba(255,255,255,.55)}
      .aguara-time-card.is-selected{border-color:#ffffff;background:#f2f2f2;color:#000;box-shadow:0 0 0 2px rgba(255,255,255,.28)}
      .aguara-time-card.is-reserved{background:rgba(198,40,40,.82);border-color:rgba(244,67,54,.95);color:#fff;cursor:not-allowed;padding-top:12px;overflow:hidden}
      .aguara-time-card.is-reserved::before{content:"✕";position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:rgba(139,0,0,.82);font-size:1.55rem;font-weight:900;line-height:1;z-index:2;pointer-events:none}
      .aguara-time-card.is-reserved small{color:#ff8989}
      @media(max-width:640px){.aguara-calendar-shell{padding:11px;border-radius:13px}.aguara-day{min-height:44px;padding:5px 3px}.aguara-day-number{font-size:.80rem}.aguara-day-label{font-size:.38rem}.aguara-time-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    `;
  ...
  function renderCalendar() {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      btn.type = "button";
      btn.className = `aguara-day${count ? " is-reserved-day" : " is-available"}${key === today ? " is-today" : ""}${key === state.selectedDate ? " is-selected" : ""}${count ? " is-reserved" : ""}`;
