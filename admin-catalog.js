(() => {
  ...
  function buildBox() {
    ...
    box.innerHTML = `<div class="catalog-admin-head"><div><h2>Editar equipamiento</h2><p>Subí fotos, editá el nombre y la descripción, encuadrá cada foto y elegí qué equipamiento mostrar en la portada. No afecta reservas ni precios.</p></div></div><div class="catalog-admin-add"><label>Nombre / modelo<input id="catalogNewName" type="text" maxlength="120" placeholder="Ej.: Tippmann Cronus"></label><label>Descripción<textarea id="catalogNewDescription" maxlength="1000" placeholder="Descripción breve para los jugadores..."></textarea></label><label>Orden<input id="catalogNewOrder" type="number" value="0" min="0"></label><div><input id="catalogNewFile" type="file" accept="image/jpeg,image/png,image/webp" hidden><button id="catalogNewButton" class="btn btn-primary" type="button">+ Agregar equipamiento</button></div></div><div id="catalogAdminStatus" class="catalog-admin-status"></div><div id="catalogAdminList" class="catalog-admin-list"></div>`;
  ...
  async function createItem(box) {
    ...
    try{const imagePath=await uploadImage(file);await api({method:"POST",body:JSON.stringify({action:"create",name,description,sortOrder,imagePath})});box.querySelector("#catalogNewName").value="";box.querySelector("#catalogNewDescription").value="";box.querySelector("#catalogNewOrder").value="0";box.querySelector("#catalogNewFile").value="";status.textContent="✓ Equipamiento agregado.";await load();}catch(error){status.textContent=error.message||"No se pudo agregar el equipamiento.";}
  }

  function render(items){
    const list=document.getElementById("catalogAdminList");if(!list)return;list.innerHTML="";
    if(!items.length){list.innerHTML='<div class="catalog-empty">Todavía no hay equipamiento cargado.</div>';return;}
    items.forEach(item=>{
  ...
  async function load(){const box=document.getElementById("catalogAdmin");if(!box)return;try{const data=await api();render(data.items||[]);}catch(error){document.getElementById("catalogAdminStatus").textContent=error.message||"No se pudo cargar el equipamiento.";}}
  ...
  async function remove(card){if(!confirm("¿Eliminar este equipamiento del sitio?"))return;const status=card.querySelector(".catalog-item-status");status.textContent="Eliminando...";try{await api({method:"DELETE",body:JSON.stringify({id:card.dataset.id})});await load();}catch(error){status.textContent=error.message||"No se pudo eliminar.";}}
  ...
  function openEditor(card){
    closeEditor();
    const img=card.querySelector(".catalog-admin-preview img");
    if(!img?.src)return;
    editor={card,zoom:1,rotation:0,x:0,y:0,dragging:false,lastX:0,lastY:0,image:null};
    const modal=document.createElement("div");modal.className="catalog-photo-editor";modal.innerHTML=`<div class="catalog-photo-editor-card"><h3>Editar foto del equipamiento</h3><p style="color:#aaa;margin-top:4px">Arrastrá la foto para encuadrarla. Podés hacer zoom y rotarla antes de guardar.</p><div class="catalog-photo-editor-canvas-wrap"><canvas width="720" height="405"></canvas></div><div class="catalog-photo-editor-controls"><label>Zoom <input class="catalog-editor-zoom" type="range" min="1" max="3" step="0.01" value="1"></label><label>Rotación <input class="catalog-editor-rotation" type="range" min="-180" max="180" step="1" value="0"></label></div><div class="catalog-photo-editor-actions"><button class="btn btn-outline catalog-editor-reset" type="button">Restablecer</button><button class="btn btn-outline catalog-editor-cancel" type="button">Cancelar</button><button class="btn btn-primary catalog-editor-save" type="button">Guardar foto editada</button></div><div class="catalog-editor-status" style="color:#aaa;margin-top:8px;min-height:18px"></div></div>`;
    document.body.appendChild(modal);
    editor.modal=modal;editor.canvas=modal.querySelector("canvas");editor.ctx=editor.canvas.getContext("2d");
    modal.querySelector(".catalog-editor-zoom").addEventListener("input",e=>{editor.zoom=Number(e.target.value);drawEditor();});
    modal.querySelector(".catalog-editor-rotation").addEventListener("input",e=>{editor.rotation=Number(e.target.value);drawEditor();});
    modal.querySelector(".catalog-editor-reset").addEventListener("click",()=>{editor.zoom=1;editor.rotation=0;editor.x=0;editor.y=0;modal.querySelector(".catalog-editor-zoom").value="1";modal.querySelector(".catalog-editor-rotation").value="0";drawEditor();});
    modal.querySelector(".catalog-editor-cancel").addEventListener("click",closeEditor);
    modal.querySelector(".catalog-editor-save").addEventListener("click",saveEditedPhoto);
    const canvas=editor.canvas;
    canvas.addEventListener("pointerdown",e=>{editor.dragging=true;editor.lastX=e.clientX;editor.lastY=e.clientY;canvas.setPointerCapture(e.pointerId);});
    canvas.addEventListener("pointermove",e=>{if(!editor.dragging)return;editor.x+=e.clientX-editor.lastX;editor.y+=e.clientY-editor.lastY;editor.lastX=e.clientX;editor.lastY=e.clientY;drawEditor();});
    canvas.addEventListener("pointerup",()=>{editor.dragging=false;});
    loadImage(img.src).then(image=>{if(!editor)return;editor.image=image;drawEditor();}).catch(error=>{if(editor)modal.querySelector(".catalog-editor-status").textContent=error.message;});
  }

  function drawEditor(){
    if(!editor?.image)return;
    const {ctx,canvas,image}=editor;ctx.clearRect(0,0,canvas.width,canvas.height);ctx.save();ctx.translate(canvas.width/2+editor.x,canvas.height/2+editor.y);ctx.rotate(editor.rotation*Math.PI/180);const scale=Math.max(canvas.width/image.width,canvas.height/image.height)*editor.zoom;ctx.drawImage(image,-image.width*scale/2,-image.height*scale/2,image.width*scale,image.height*scale);ctx.restore();}
