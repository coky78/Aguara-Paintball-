(() => {
  ...
  async function uploadImage(file) {
    ...
    if (!uploadResponse.ok) {
      let detail = "";
      try { detail = (await uploadResponse.json())?.message || (await uploadResponse.text()); } catch (_) {}
      throw new Error(detail ? `No se pudo subir la imagen: ${detail}` : `No se pudo subir la imagen (HTTP ${uploadResponse.status}).`);
    }
